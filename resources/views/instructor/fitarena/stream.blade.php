    ...
    <div class="d-flex flex-column h-100">
        ...
        <div class="bg-dark text-white p-3 d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex align-items-center gap-3">
                ...
                <a href="{{ route('instructor.fitarena.sessions') }}" class="btn btn-outline-light btn-sm">
            ...
            <div class="bg-dark text-white" style="width: 350px;">
                <div class="p-3">
                    ...
                    <div class="mb-3">
                        ...
                        <span class="text-muted">{{ $session->event->title }}</span>
                    </div>
                ...
                <div class="border-top border-secondary p-3">
                    ...
                    <div class="d-grid gap-2">
                        <a href="{{ route('fitarena.session', [$session->event->id, $session->id]) }}" class="btn btn-outline-light btn-sm" target="_blank">
                            <i class="fas fa-external-link-alt me-1"></i> View Public Page
                        </a>
                        <button class="btn btn-outline-info btn-sm" onclick="shareSession()">
                            <i class="fas fa-share me-1"></i> Share Session
                        </button>
                    </div>
                </div>
            </div>
...
async function startStream() {
    try {
        ...
        const response = await fetch(`{{ route('instructor.fitarena.sessions.stream.update', [$session->id, 'start']) }}`, {
            method: 'POST',
            headers: {
...
async function endStream() {
    try {
        ...
        // Update session status
        const response = await fetch(`{{ route('instructor.fitarena.sessions.stream.update', [$session->id, 'end']) }}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': CONFIG.csrf
...
function shareSession() {
    const url = '{{ route("fitarena.session", [$session->event->id, $session->id]) }}';
    if (navigator.share) {
        navigator.share({
            title: '{{ $session->title }}',
            text: 'Join my FitArena session!',
            url: url
        });
    } else {
