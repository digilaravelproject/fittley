...
<div class="fitdoc-single-index">
    ...
    <div class="page-header mb-5">
        <div class="row align-items-center">
            ...
            <div class="col-md-4 text-end">
                <a href="{{ route('admin.fitdoc.single.create') }}" class="btn btn-primary">
    ...
    <div class="content-card mb-4">
        <div class="content-card-body">
            <form method="GET" action="{{ route('admin.fitdoc.single.index') }}" class="row g-3">
                <div class="col-md-4">
                ...
                <div class="col-md-2">
                    <div class="form-group">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <a href="{{ route('admin.fitdoc.single.index') }}" class="btn btn-outline-secondary">
    ...
    <div class="content-card">
        <div class="content-card-header">
            ...
            <p class="content-card-subtitle">{{ $singles->total() ?? 0 }} videos found</p>
        </div>
        <div class="content-card-body p-0">
            @if($singles && $singles->count() > 0)
                    ...
                    <table class="modern-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <div class="video-info">
                                        <div class="video-icon">
                                            @if($single->video_type === 'youtube')
                                                <i class="fab fa-youtube text-danger"></i>
                                            @elseif($single->video_type === 's3')
                                        ...
                                        <div class="video-details">
                                            <div class="video-title">{{ $single->title }}</div>
                                            <div class="video-description">{{ Str::limit($single->description, 50) }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <div class="language-cost">
                                        <div class="language">{{ ucfirst($single->language) }}</div>
                                        <div class="cost">{{ $single->cost > 0 ? '$' . number_format($single->cost, 2) : 'Free' }}</div>
                                    </div>
                                </td>
                                <td>
                                    <div class="duration-info">
                                        <i class="fas fa-clock me-1"></i>
                                        {{ $single->formatted_duration ?? '0h 12m' }}
                                    </div>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="date-info">
                                        <div class="date-primary">{{ $single->created_at->format('M j, Y') }}</div>
                                        <div class="date-secondary">{{ $single->created_at->format('g:i A') }}</div>
                                    </div>
                                </td>
                                <td>
                                    <div class="action-buttons">
                                        <a href="{{ route('admin.fitdoc.single.show', $single) }}" 
                                           class="action-btn action-btn-primary" 
                                           title="View Video">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{ route('admin.fitdoc.single.edit', $single) }}" 
                                        ...
                                        <form method="POST" 
                                              action="{{ route('admin.fitdoc.single.destroy', $single) }}" 
                                              class="d-inline"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.toggle-status-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            fetch(`{{ route('admin.fitdoc.single.index') }}/${videoId}/toggle-status`, {
                method: 'PATCH',
                headers: {
