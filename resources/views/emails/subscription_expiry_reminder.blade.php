...
<body>
    ...
    <div class="container">
        ...
        <!-- Main Content -->
        <h2>Upcoming Subscription Expiry</h2>
        <p class="content">Hi {{ $subscription->user->name ?? $subscription->user->email }},</p>

        <p class="content">
            Your subscription to <strong>{{ optional($subscription->subscriptionPlan)->name }}</strong> will expire in
            <strong>{{ $daysLeft }}</strong> day(s) on
            <strong>{{ $subscription->ends_at->toDayDateTimeString() }}</strong>.
        </p>

        <p class="content">
            To continue enjoying our services without interruption, we encourage you to renew or upgrade your
            subscription today.
        </p>

        <!-- Call to Action Button -->
        <a href="{{ url('/subscription/plans') }}" class="cta-button">Renew / Upgrade now</a>
