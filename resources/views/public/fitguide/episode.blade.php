...
<div class="container-fluid bg-dark text-white min-vh-100">
    <!-- Hero Section -->
    <div class="hero-section py-5" style="background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.8)), url('{{ $fgSeries->banner_image_url ?? 'https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?ixlib=rb-4.0.3' }}') center/cover;">
        <div class="container">
            <div class="row align-items-center">
                <div class="col-lg-8">
                    ...
                    <nav aria-label="breadcrumb" class="mb-3">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ route('fitguide.index') }}" class="text-light">FitGuide</a></li>
                            <li class="breadcrumb-item"><a href="{{ route('fitguide.series.show', $fgSeries->slug) }}" class="text-light">{{ $fgSeries->title }}</a></li>
                            <li class="breadcrumb-item active text-warning" aria-current="page">Episode {{ $episode->episode_number }}</li>
                        </ol>
                    </nav>
                    ...
                    <div class="d-flex gap-3 flex-wrap">
                        ...
                        @endif
                        <a href="{{ route('fitguide.series.show', $fgSeries->slug) }}" class="btn btn-outline-light btn-lg">
    ...
    <div class="related-section py-5 bg-secondary">
        <div class="container">
            <h2 class="section-title mb-4">
                <i class="fas fa-list me-2"></i>{{ $fgSeries->title }} - All Episodes
            </h2>
            <div class="row">
                @foreach($fgSeries->episodes->where('is_published', true) as $ep)
                    <div class="col-lg-3 col-md-6 mb-4">
                        <div class="content-card bg-dark rounded overflow-hidden {{ $ep->id === $episode->id ? 'border border-primary' : '' }}">
                            <img src="{{ $fgSeries->banner_image_url ?? 'https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?ixlib=rb-4.0.3' }}" 
                                 class="card-img-top" style="height: 200px; object-fit: cover;" alt="Episode {{ $ep->episode_number }}">
                            <div class="card-body">
                                <h6 class="card-title">Episode {{ $ep->episode_number }}: {{ $ep->title }}</h6>
                                <p class="card-text small text-muted">{{ Str::limit($ep->description ?? 'Episode description', 80) }}</p>
                                @if($ep->id === $episode->id)
                                    <span class="btn btn-primary btn-sm w-100">
                                        <i class="fas fa-play me-1"></i>Currently Watching
                                    </span>
                                @else
                                    <a href="{{ route('fitguide.series.episode', [$fgSeries->slug, $ep->episode_number]) }}" class="btn btn-outline-primary btn-sm w-100">
