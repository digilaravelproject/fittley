    ...
    <section class="hero-section bg-dark text-white position-relative overflow-hidden">
        ...
        <div class="hero-content position-absolute top-50 start-50 translate-middle text-center w-100">
            <div class="container">
                ...
                <h1 class="display-4 fw-bold mb-3">{{ $heroEvent->title }}</h1>
                <p class="lead mb-4">{{ $heroEvent->description }}</p>
                <div class="d-flex justify-content-center gap-3 flex-wrap">
                    <a href="{{ route('fitarena.event', $heroEvent->slug) }}" class="btn btn-primary btn-lg">
                ...
                <div class="col-lg-4 col-md-6">
                    <div class="card event-card h-100 border-0 shadow-sm bg-dark text-white">
                        ...
                        <div class="card-body">
                            ...
                            <p class="card-text text-light small">{{ Str::limit($event->description, 100) }}</p>
                            <div class="d-flex justify-content-between align-items-center">
                                <small class="text-light">
                                    <i class="fas fa-calendar me-1"></i>{{ $event->start_date->format('M j') }}
                                </small>
                                <span class="badge bg-success">{{ $event->event_type ?? 'Competition' }}</span>
                            </div>
                        </div>
                    ...
                    <div class="card event-card h-100 border-0 shadow-sm bg-dark text-white">
                        <div class="position-relative">
                            ...
                            <span class="position-absolute top-0 start-0 m-2 badge bg-warning text-dark">
                                <i class="fas fa-clock me-1"></i>{{ $event->getDaysUntilStart() }}d
                            </span>
                        </div>
                        <div class="card-body">
                            <h5 class="card-title text-white">{{ $event->title }}</h5>
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                <small class="text-light">
                                    <i class="fas fa-calendar me-1"></i>{{ $event->start_date->format('M j, Y') }}
                                </small>
                                <span class="badge bg-info">{{ $event->event_type ?? 'Competition' }}</span>
                            </div>
                        </div>
                        <div class="card-footer bg-transparent border-0">
                            <a href="{{ route('fitarena.event', $event->slug) }}" class="btn btn-outline-primary w-100">
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-3 col-md-6">
                    <div class="card event-card h-100 border-0 shadow-sm bg-dark text-white">
                        ...
                        <div class="card-body">
                            <h6 class="card-title text-white">{{ $event->title }}</h6>
                            <small class="text-light d-block mb-2">
                                <i class="fas fa-calendar me-1"></i>{{ $event->start_date->format('M j') }}
                            </small>
                            <span class="badge bg-{{ $event->status_color }}">{{ ucfirst($event->status) }}</span>
                        </div>
                        <div class="card-footer bg-transparent border-0 p-2">
                            <a href="{{ route('fitarena.event', $event->slug) }}" class="btn btn-sm btn-outline-primary w-100">
                                View Event
                            </a>
                        </div>
                    </div>
