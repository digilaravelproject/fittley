...
<div class="fitarena-page">
    ...
    <section class="hero-section position-relative overflow-hidden">
        ...
        @if($heroEvent->banner_image)
            <img src="{{ getImagePath($heroEvent->banner_image) }}" alt="{{ $heroEvent->title ?? 'Live Event' }}" class="hero-bg-image">
        @else
    ...
    <section class="live-section py-5">
        <div class="container">
            <div class="section-header text-center mb-4">
                ...
                <p class="section-subtitle text-muted">Join the action happening right now</p>
            </div>
            <div class="row g-4">
                    ...
                    <div class="card event-card-glass h-100 border-0 overflow-hidden">
                        <div class="card-img-wrapper position-relative">
                            @if($event->banner_image)
                                <img src="{{ getImagePath($event->banner_image) }}" alt="{{ $event->title }}" class="card-img-top">
                            @else
                        ...
                        <div class="card-body">
                            ...
                            <div class="d-flex justify-content-between align-items-center mt-3">
                                <small><i class="fas fa-calendar me-1"></i>{{ $event->start_date->format('M j') }}</small>
    ...
    <section class="upcoming-section py-5 bg-light">
        <div class="container">
            <div class="section-header text-center mb-4">
                ...
                <p class="section-subtitle text-muted">Don't miss these exciting competitions</p>
            </div>
            <div class="row g-4">
                ...
                <div class="col-lg-4 col-md-6">
                    <div class="card event-card h-100 border-0 shadow-sm">
                        <div class="card-img-wrapper position-relative">
                            ...
                            @endif
                            <span class="badge badge-uptime top-left"><i class="fas fa-clock me-1"></i>{{ $event->getDaysUntilStart() }}d</span>
                        </div>
                        <div class="card-body">
                            <h5 class="card-title">{{ $event->title }}</h5>
                            <p class="card-text">{{ Str::limit($event->description, 100) }}</p>
                            <div class="d-flex justify-content-between align-items-center mt-3">
                                <small><i class="fas fa-calendar me-1"></i>{{ $event->start_date->format('M j, Y') }}</small>
                ...
                <div class="col-lg-3 col-md-6">
                    <div class="card event-card-glass-sm h-100 border-0 overflow-hidden">
                        ...
                        <div class="card-body">
                            ...
                            <small class="d-block mb-2"><i class="fas fa-calendar me-1"></i>{{ $event->start_date->format('M j') }}</small>
                            <span class="badge badge-status">{{ ucfirst($event->status) }}</span>
                        </div>
                        <div class="card-footer bg-transparent border-0 p-2">
                            <a href="{{ route('fitarena.event', $event->slug) }}" class="btn btn-outline-light w-100 btn-sm">
                                View Event
                            </a>
                        </div>
                    </div>
